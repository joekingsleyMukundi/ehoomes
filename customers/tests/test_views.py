...
class TestViews(TestCase):
    def setUp(self):
        ...
        self.user = users_model.objects.create(username='test', user_phone='123456789', email='joekingsleymukundi@gmail.com',)
    ...
    def test_user_profile_view_POST(self):
        self.client.defaults['HTTP_AUTHORIZATION'] = 'Bearer ' + self.tokens.json()['access']
        response = self.client.post(reverse('user_profile'),{'username': 'test', 'user_phone': '123456789', 'email': 'test@gmail.com'},)
        self.assertEquals(response.data['username'], 'test')
        self.assertEquals(response.data['user_phone'], '123456789')
